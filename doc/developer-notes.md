...
- [Development guidelines](#development-guidelines)
    - [General BogCoin Core](#general-bogcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Run with the `-testnet` option to run with "play BOGs (tBOG)" on the test network, if you
...
BogCoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/bogcoin-official/bogcoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_bogcoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_bogcoin --log_level=test_suite
$ valgrind -v --leak-check=full src/bogcoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_bogcoin.coverage/index.html`.
```

**Sanitizers**

BogCoin Core can be compiled with various "sanitizers" enabled, which add
 ...
 * [Issue #12691: Enable -fsanitize flags in Travis](https://github.com/bogcoin/bogcoin/issues/12691)
...
- BogcoinMiner : Generates bogcoins (if wallet is enabled).
...
However, in open source software such as BogCoin Core, where everyone uses
...
pay attention to for reviewers of BogCoin Core code.

General BogCoin Core
--------------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: BogCoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/bar.h` should use the include guard identifier `BOGCOIN_FOO_BAR_H`, e.g.

```c++
#ifndef BOGCOIN_FOO_BAR_H
#define BOGCOIN_FOO_BAR_H
...
#endif // BOGCOIN_FOO_BAR_H
```
...
Some of these are maintained by active developers of BogCoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against BogCoin Core so that they can be integrated
...
- src/libsecp256k1
  - Upstream at https://github.com/bogcoin-core/secp256k1/ ; actively maintained by Core contributors.

- src/univalue
  - Upstream at https://github.com/bogcoin-core/univalue ; actively maintained by Core contributors, deviates from upstream https://github.com/jgarzik/univalue
...
file descriptors it will cause problems with BogCoin's `select()` loop, because
...
compatibility between nodes. This happened in Bogcoin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to BogCoin's
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:bogcoin/bogcoin.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `bogcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `bogcoin-cli` and the GUI debug console use this table to determine how to
