...
'''
Test plan:
- Start bogcoind's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on bogcoind side:
...
- Create bogcoinds that connect to them
- Manipulate the bogcoinds using addnode (onetry) an observe effects
...
class ProxyTest(BogcoinTestFramework):
    def __init__(self):
    ...
    def node_test(self, node, proxies, auth):
        ...
        # Note: bogcoind's SOCKS5 implementation only sends atyp DOMAINNAME, even if connecting directly to IPv4/IPv6
        ...
        node.addnode("youraddress.onion:9333", "onetry")
