...
if [ $# -lt 1 ]; then
        ...
        echo "e.g. $0 ../../src"
        echo "Env vars BOGCOIND and BOGCOINCLI may be used to specify the exact binaries used"
        exit 1
fi

set -f

BOGCOIND=${BOGCOIND:-${1}/bogcoind}
CLI=${BOGCOINCLI:-${1}/bogcoin-cli}
...
$BOGCOIND -zapwallettxes=1 $B1ARGS &
B1PID=$!
$BOGCOIND -zapwallettxes=2 $B2ARGS &
B2PID=$!
