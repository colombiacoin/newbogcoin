                                                                    ...
                                                                    model(0)
{
    ...
    ui->signatureOut_SM->setFont(GUIUtil::bogcoinAddressFont());
    ui->signatureIn_VM->setFont(GUIUtil::bogcoinAddressFont());
}
...
void SignVerifyMessageDialog::on_signMessageButton_SM_clicked()
{
    ...
    CBogcoinAddress addr(ui->addressIn_SM->text().toStdString());
    if (!addr.IsValid()) {
...
{
    CBogcoinAddress addr(ui->addressIn_VM->text().toStdString());
    if (!addr.IsValid()) {
    ...
    if (!(CBogcoinAddress(pubkey.GetID()) == addr)) {
        ui->statusLabel_VM->setStyleSheet("QLabel { color: red; }");
