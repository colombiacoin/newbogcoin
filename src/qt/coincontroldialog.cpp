                                                        ...
                                                        model(0)
{
    ...
    // see https://github.com/bogcoin/bogcoin/issues/5716
...
{
    GUIUtil::setClipboard(BogcoinUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(WalletModel* model, QDialog* dialog)
{
    ...
    int nDisplayUnit = BogcoinUnits::BOG;
    ...
    l2->setText(BogcoinUnits::formatWithUnit(nDisplayUnit, nAmount));   // Amount
    l3->setText(BogcoinUnits::formatWithUnit(nDisplayUnit, nPayFee));   // Fee
    l4->setText(BogcoinUnits::formatWithUnit(nDisplayUnit, nAfterFee)); // After Fee
    l5->setText(((nBytes > 0) ? "~" : "") + QString::number(nBytes));   // Bytes
    l6->setText(sPriorityLabel);                                        // Priority
    l7->setText(fDust ? tr("yes") : tr("no"));                          // Dust
    l8->setText(BogcoinUnits::formatWithUnit(nDisplayUnit, nChange));   // Change
    ...
    toolTip1 += tr("This means a fee of at least %1 per kB is required.").arg(BogcoinUnits::formatWithUnit(nDisplayUnit, CWallet::minTxFee.GetFeePerK())) + "<br /><br />";
    toolTip1 += tr("Can vary +/- 1 byte per input.");

    QString toolTip2 = tr("Transactions with higher priority are more likely to get included into a block.") + "<br /><br />";
    toolTip2 += tr("This label turns red, if the priority is smaller than \"medium\".") + "<br /><br />";
    toolTip2 += tr("This means a fee of at least %1 per kB is required.").arg(BogcoinUnits::formatWithUnit(nDisplayUnit, CWallet::minTxFee.GetFeePerK()));

    QString toolTip3 = tr("This label turns red, if any recipient receives an amount smaller than %1.").arg(BogcoinUnits::formatWithUnit(nDisplayUnit, ::minRelayTxFee.GetFee(546)));
    ...
    QString toolTip4 = tr("Can vary +/- %1 ubog per input.").arg(dFeeVary);
    ...
    BOOST_FOREACH (PAIRTYPE(QString, vector<COutput>) coins, mapCoins) {
        ...
        for(const COutput& out: coins.second) {
            ...
            if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress)) {
                sAddress = QString::fromStdString(CBogcoinAddress(outputAddress).ToString());

                // if listMode or change => show BOG address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, BogcoinUnits::format(nDisplayUnit, out.tx->vout[out.i].nValue));
            itemOutput->setToolTip(COLUMN_AMOUNT, BogcoinUnits::format(nDisplayUnit, out.tx->vout[out.i].nValue));
        ...
        if (treeMode) {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, BogcoinUnits::format(nDisplayUnit, nSum));
            itemWalletAddress->setToolTip(COLUMN_AMOUNT, BogcoinUnits::format(nDisplayUnit, nSum));
