...
const QString BogcoinGUI::DEFAULT_WALLET = "~Default";

BogcoinGUI::BogcoinGUI(const NetworkStyle* networkStyle, QWidget* parent) : QMainWindow(parent),
                                                                            clientModel(0),
                                                                            walletFrame(0),
                                                                            ...
                                                                            spinnerFrame(0)
{
    ...
    QString windowTitle = tr("BogCoin Core") + " - ";
#ifdef ENABLE_WALLET
...
void BogcoinGUI::createActions(const NetworkStyle* networkStyle)
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a BOG address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and bogcoin: URIs)"));
    ...
    aboutAction = new QAction(networkStyle->getAppIcon(), tr("&About BogCoin Core"), this);
    aboutAction->setStatusTip(tr("Show information about BogCoin Core"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/qt-project.org/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setStatusTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setStatusTip(tr("Modify configuration options for BogCoin"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your BOG addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/transaction_black"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified BOG addresses"));
    ...
    openAction->setStatusTip(tr("Open a bogcoin: URI or payment request"));
    openBlockExplorerAction = new QAction(QIcon(":/icons/explorer"), tr("&Blockchain explorer"), this);
    openBlockExplorerAction->setStatusTip(tr("Block explorer window"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the BogCoin Core help message to get a list with possible BogCoin command-line options"));
...
void BogcoinGUI::createToolBars()
{
    if (walletFrame) {
...
void BogcoinGUI::setClientModel(ClientModel* clientModel)
{
...
#ifdef ENABLE_WALLET
bool BogcoinGUI::addWallet(const QString& name, WalletModel* walletModel)
{
    if (!walletFrame)
        return false;
...
bool BogcoinGUI::setCurrentWallet(const QString& name)
{
    if (!walletFrame)
        return false;
...
void BogcoinGUI::removeAllWallets()
{
    if (!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void BogcoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void BogcoinGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
#ifndef Q_OS_MAC
    trayIcon = new QSystemTrayIcon(this);
    QString toolTip = tr("BogCoin Core client") + " " + networkStyle->getTitleAddText();
...
void BogcoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void BogcoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void BogcoinGUI::optionsClicked()
{
...
void BogcoinGUI::showHelpMessageClicked()
{
...
void BogcoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void BogcoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void BogcoinGUI::gotoProposalPage()
{
    QSettings settings;
    if (settings.value("fShowBudgetProposalsTab").toBool() || settings.value("fShowCommunityProposalsTab").toBool())
    {
...
void BogcoinGUI::gotoMasternodePage()
{
    QSettings settings;
    if (settings.value("fShowMasternodesTab").toBool()) {
...
void BogcoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void BogcoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void BogcoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void BogcoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

void BogcoinGUI::gotoMultisigCreate()
{
    if(walletFrame) walletFrame->gotoMultisigDialog(0);
}

void BogcoinGUI::gotoMultisigSpend()
{
    if(walletFrame) walletFrame->gotoMultisigDialog(1);
}

void BogcoinGUI::gotoMultisigSign()
{
    if(walletFrame) walletFrame->gotoMultisigDialog(2);
}

void BogcoinGUI::gotoBip38Tool()
{
    if (walletFrame) walletFrame->gotoBip38Tool();
}

void BogcoinGUI::gotoMultiSendDialog()
{
    multiSendAction->setChecked(true);
    if (walletFrame)
        walletFrame->gotoMultiSendDialog();
}
void BogcoinGUI::gotoBlockExplorerPage()
{
...
void BogcoinGUI::setNumConnections(int count)
{
    QString icon;
    switch (count) {
    case 0:
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to BogCoin network", "", count));
}

void BogcoinGUI::setNumBlocks(int count)
{
    if (!clientModel)
        return;
    ...
    //    if(secs < 25*60) // 90*60 for bogcoin but we are 4x times faster
...
void BogcoinGUI::message(const QString& title, const QString& message, unsigned int style, bool* ret)
{
    QString strTitle = tr("BogCoin Core"); // default title
...
void BogcoinGUI::changeEvent(QEvent* e)
{
...
void BogcoinGUI::closeEvent(QCloseEvent* event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if (clientModel && clientModel->getOptionsModel()) {
...
#ifdef ENABLE_WALLET
void BogcoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address)
{
    ...
    if(!bdisableSystemnotifications){
        ...
        message((amount) < 0 ? (pwalletMain->fMultiSendNotify == true ? tr("Sent MultiSend transaction") : tr("Sent transaction")) : tr("Incoming transaction"),
            tr("Date: %1\n"
               ...
               "Address: %4\n")
                .arg(date)
                .arg(BogcoinUnits::formatWithUnit(unit, amount, true))
                .arg(type)
                .arg(address),
...
void BogcoinGUI::dragEnterEvent(QDragEnterEvent* event)
{
    // Accept only URIs
...
void BogcoinGUI::dropEvent(QDropEvent* event)
{
    if (event->mimeData()->hasUrls()) {
...
bool BogcoinGUI::eventFilter(QObject* object, QEvent* event)
{
...
#ifdef ENABLE_WALLET
void BogcoinGUI::setStakingStatus()
{
    if (pwalletMain)
...
bool BogcoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void BogcoinGUI::setEncryptionStatus(int status)
{
    switch (status) {
...
void BogcoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if (!clientModel)
        return;
...
void BogcoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void BogcoinGUI::detectShutdown()
{
...
void BogcoinGUI::showProgress(const QString& title, int nProgress)
{
    if (nProgress == 0) {
...
static bool ThreadSafeMessageBox(BogcoinGUI* gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void BogcoinGUI::subscribeToCoreSignals()
{
    // Connect signals to client
    uiInterface.ThreadSafeMessageBox.connect(boost::bind(ThreadSafeMessageBox, this, _1, _2, _3));
}

void BogcoinGUI::unsubscribeFromCoreSignals()
{
...
void BogcoinGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
...
{
    menu = new QMenu();
    foreach (BogcoinUnits::Unit u, BogcoinUnits::availableUnits()) {
        QAction* menuAction = new QAction(QString(BogcoinUnits::name(u)), this);
...
void UnitDisplayStatusBarControl::updateDisplayUnit(int newUnits)
{
    if (Params().NetworkID() == CBaseChainParams::MAIN) {
        setPixmap(QIcon(":/icons/unit_" + BogcoinUnits::id(newUnits)).pixmap(39, STATUSBAR_ICONSIZE));
    } else {
        setPixmap(QIcon(":/icons/unit_t" + BogcoinUnits::id(newUnits)).pixmap(39, STATUSBAR_ICONSIZE));
    }
}
