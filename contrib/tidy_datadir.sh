#!/bin/bash
# Copyright (c) 2013 The Bogcoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete BogCoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no BogCoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old BogCoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected BogCoin 0.7 datadir."
    ;;
  3)
    echo "Detected BogCoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected BogCoin 0.8 datadir."
    ;;
esac
